...
void pad_chr(char chr, unsigned char dir, unsigned char padded_length, char *str)
{
	...
	if((len > 0) && (padded_length > len)) 
	{
		if(dir == SLEFT) 
		{
			/* right justify the current string in the space! */
			src += len; /* point to NULL */
			dst += padded_length;
			for(i=0;i<=len;src--,dst--,i++) 
			{
				*dst = *src;
			}
			/* pad the given pad char on the left of the string now */
			while(dst != str) *dst-- = chr;
			*dst = chr;
		}
		else 
		{
			/* just add in the reqd number of pad chars at the end of string */
			dst += len;
			for(i=0;i<(padded_length-len);*dst++ = chr, i++);
			*dst = '\0';
		}
	}
}
